...
async function createAdminAndHotel() {
  ...
  const adminLoginResponse = await supertest(app).post("/api/auth/login").send({
    username: "adminUser",
    password: "adminPass123",
  });

  const adminToken = adminLoginResponse.headers["set-cookie"][0].replace(
    "; Path=/; HttpOnly",
    ""
  );
...
async function createUser() {
  ...
  const userLoginResponse = await supertest(app).post("/api/auth/login").send({
    username: "regularUser",
    password: "userPass123",
  });
  const userToken = userLoginResponse.headers["set-cookie"][0].replace(
    "; Path=/; HttpOnly",
    ""
  );
  return userToken;
...
describe("RTC_005", () => {
  it("should get details of a specific room", async () => {
    ...
    const response = await supertest(app).get(`/api/rooms/${roomId}`);
...
describe("RTC_006", () => {
  it("should return 404 for a non-existent room", async () => {
    const nonExistentRoomId = "5fb9957a9b0eaf001e25b6e4";
    const response = await supertest(app)
